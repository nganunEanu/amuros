...
# Open serial port
serialPort = rm.open_resource("ASRL/dev/ttyUSB1::INSTR")
    ...
    for r in result1:
        ...
        for box in boxes:
            ...
            if (angle_degrees0 < 180):
                serialPort.write_raw(f"putar kiri".encode('utf-8'))
                print("Putar kiri")
            elif (angle_degrees0 > 180):
                serialPort.write_raw(f"putar kanan".encode('utf-8'))
                print("Putar kanan")
                

            if (berhenti_maju_x1 < center_x_box1 < berhenti_maju_x2 and berhenti_maju_y1 < center_y_box1 < berhenti_maju_y2):
                print("Berhenti maju")
                serialPort.write_raw(f"berhenti maju".encode('utf-8'))
            else:
                print("Maju")
                serialPort.write_raw(f"maju".encode('utf-8'))
