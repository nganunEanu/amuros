...
void loop() {
  ...
  if (langkahPos == 2 && odometryStop == 0)
  {
    if (PIDodometry == 4){
      PIDpos(YYY, XXX, TTT, SPD);
    }
  }
  ...
  {
    if (PIDodometry == 0) {
      PIDpos(0, -1850, 0, 4); //Memanggil fungsi PID Posisi dengan Koreksi ODOMETRY
    }
    else if (PIDodometry == 1) {
      PIDpos(2200, -1850, 0, 4); //Memanggil fungsi PID Posisi dengan Koreksi ODOMETRY
    }
  }
  //    if(modes==2)
  //    {
...
void PID(int inp, int qw) {
  ...
  if (target[qw] == 0) {
    analogWrite(pwmm[m[qw]], 0);
  }
  else {
    analogWrite(pwmm[m[qw]], pwr);
  }
