...
void loop() {
  ...
  mil = millis();
  mil1 = millis();
  if(mil1 - milseb11 >=200){
      ///////////-------------------------------------KINEMATIKA-----------------------

  if(dones==0){PIDpos(1000, 1000, 0, 1);}
  else if(dones==1){PIDpos(0, 0, 90, 1);}
  else if(dones==2){PIDpos(500, 200, 0, 1);}
  else if(dones==3){PIDpos(0, 0, 20, 1);}
  else if(dones==4){PIDpos(0, 0, 0, 1);}
  //  if(t < 120) 
  //  {
    //    PIDpos(300, 0, -90, 0.8);
...
//    float  y1 = (y/376.8)*60;
    ...
    float  x1 = x * spd;
    float  y1 = y * spd;
    float thet1 = thet * spd;

      a = -0.33;  b = 0.577;   c = 0.33;
      d = -0.33;  e = -0.577;  f = 0.33;
      g = 0.67;  h = 0;      i = 0.33;

        mot0 = (a*x1 + b*y1  + c*thet1);
        mot1 = (d*x1 + e*y1  + f*thet1);
        mot2 = (g*x1 + h*y1  + i*thet1);

         target[0] = mot0;
         target[1] = mot1;
         target[2] = mot2;
//        for(int i=0; i<3; i++){
//            if(target[i]<2 && target[i]>-2)target[i]=0;
//          }
...
void PIDpos(int y,int x, int tthet, float speedd){
  float kpP = 0.2;
    
  int errx = x - Xpos;
  int erry = y - Ypos;
  int errthet = tthet - Thet;

  float ux = kpP*errx;
  float uy = kpP*erry;
  float uthet = (1 * errthet);
...
//  }
  ...
  if(y!=0 && tthet==0 || x!=0 && tthet==0 || y!=0 && x!=0 && tthet==0 || y==0 && x==0 && tthet==0 ){kinematik(ux, uy, 0, speedd);
...
//      if (job == 1)conjob =1;
  ...
  if(y==0 && tthet!=0 || x==0 && tthet!=0 || y==0 && x==0 && tthet!=0 || y!=0 && x!=0 && tthet!=0 ){kinematik(0, 0, -uthet, speedd);
      if(errthet<8 && errthet>-8 ){
   //   kinematik(0, 0, 0, 0);
      dones++;
//      job = 1;
//      if(job == 2) job =3;
...
//  if(errx<15 && errx>-15 && erry<15 && erry>-15 && errthet<8 && errthet>-8){
//      kinematik(0, 0, 0, 0);
//      dones++;
////      job = 1;
////      if(job == 2) job =3;
...
//    }
  ...
  // else   kinematik(ux, uy, uthet, speedd);
