...
const int in1[] = {23,21,14};
const int in2[] = {22,20,15};
//const int pwmm[] = {2,3,4};
...
//    float kp = 0.5, kd=0.5, ki=0.08;
               ...
               if(target[qw]>0){
                digitalWrite(in1[m[qw]],1);
                digitalWrite(in2[m[qw]],0); 
                Serial.println('b');   
                }
            
               else if(target[qw]<0){
                digitalWrite(in1[m[qw]],0);
                digitalWrite(in2[m[qw]],1);  
                Serial.println('c');    
                }
               else{
...
target[2] = target[2] * variableMax;
          for(int qw=0; qw<3; qw++){
               ...
               analogWrite(pwmm[m[qw]], target[qw]); 
          }  
